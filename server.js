        ...
        ws.onmessage = function(event) {
            if (event.data instanceof Blob) {
                ...
                try {
                    ...
                    if (data.Mode === "NORMAL") { modeText = "🟢 一般模式"; modeClass = "mode-normal"; } 
                    else if (data.Mode === "CROWD") { modeText = "🟠 人潮擁擠"; modeClass = "mode-crowd"; } 
                    else if (data.Mode === "MUTED") { modeText = "🔴 靜音中"; modeClass = "mode-muted"; }
...
const server = http.createServer((req, res) => {
    // 當使用者打開網址時，回傳上面的 HTML
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    res.end(htmlContent);
});

// 3. 建立 WebSocket 伺服器 (綁定在 HTTP 伺服器上)
